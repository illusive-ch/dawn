<template>
  ...
  >
      ...
      <div class="result__images__container">
        <figure class="result__images__container__figure">
          <img
            ...
            alt="Before"
          />
          <figcaption class="result__images__container__figure__caption">
            Before
          </figcaption>
        </figure>
        <figure class="result__images__container__figure">
    ...
    <div class="result__body">
      ...
      <p class="result__body__about" v-text="description"></p>
    </div>
    <div
    ...
    >
      <template v-if="concern.ingredients.length !== 0 && hasIngredients">
        ...
        <div class=""  style="display: block; position: relative; width: 100%">
          <carousel :settings="settings" :breakpoints="breakpoints">
            <template
              v-for="(ingredient, index) in concern.ingredients.filter(
                (value, index, self) =>
            ...
            >
              <slide v-if="ingredient.image">
                <div class="carousel__item">
                  <img class="carousel__item__image" :src="ingredient.image" />
                  <!-- <img :src="ingredient.image"/>-->
                  <span
                    class="carousel__item__title"
                    v-text="ingredient.ConsumerTitle"
                  ></span>
                </div>
              </slide>
            </template>
...
export default {
  ...
  computed: {
    ...
    title() {
      return this.$data[this.lookupTitle]
        ? this.$data[this.lookupTitle].title
        : this.product.title;
    },
    beforeImage() {
      return this.$data[this.lookupTitle]
        ? this.$data[this.lookupTitle].img_before
        : false;
    },
    afterImage() {
      return this.$data[this.lookupTitle]
        ? this.$data[this.lookupTitle].img_after
        : false;
    },
    description() {
      return this.$data[this.lookupTitle]
        ? this.$data[this.lookupTitle].description
        : "";
    },
    percent() {
      return this.$data[this.lookupTitle]
        ? this.$data[this.lookupTitle].clinical_percent
        : false;
    },
    percentBody() {
      return this.$data[this.lookupTitle]
        ? this.$data[this.lookupTitle].clinical_text
        : false;
    },
    show() {
      return this.$data[this.lookupTitle]
        ? this.$data[this.lookupTitle].display
        : false;
    },
    hasIngredients() {
